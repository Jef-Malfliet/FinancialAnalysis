...
public class CompareReportService extends ReportService {
    ...
    protected void createInternal() {
        ...
        if (reportType.equals(ReportType.VERGELIJKINGBVBA)) {
        ...
        //TODO: fix de indentatie van deze onderdelen.
    ...
    private void addCompareReportHeader() {
        XSSFRow nameRow = addRow(reportSheet);
        skipCell(2);
        ...
        XSSFRow yearRow = addRow(reportSheet);
        skipCell(2);
    ...
    private void addCompareReportRow(String rowName, Function<Integer, String> formulaFunction, XSSFCellStyle valueStyle) {
        addCompareReportRow(rowName, formulaFunction, CellType.NUMERIC, d -> valueStyle, null);
    }
    ...
    private void addCompareReportRow(String rowName, Function<Integer, String> formulaFunction, XSSFCellStyle valueStyle, XSSFCellStyle titleStyle) {
        addCompareReportRow(rowName, formulaFunction, CellType.NUMERIC, d -> valueStyle, titleStyle);
    }

    private void addCompareReportRow(String rowName, Function<Integer, String> formulaFunction, CellType cellType,
                                     Function<Integer, XSSFCellStyle> valueStyleFunction,
                                     XSSFCellStyle titleStyle) {
        XSSFRow row = addRow(reportSheet);

        skipCell();
        addCell(row, rowName, titleStyle);

        if (formulaFunction != null) {
            skipCell();

            for (int i = 0; i < documentCount; i++) {
                addReferenceCell(row, formulaFunction.apply(i), valueStyleFunction, cellType);
            }
        }
    }
